...
# тяжесть симптомов
 SEVERITY = pd.read_csv(PATH + 'my_data.my_Symptom_severity.csv', index_col='Symptom')


# описание болезни
DESCRIPTTIONS = pd.read_csv(
    PATH + 'my_data.my_symptom_description.csv', index_col="Disease"
    )


# порядок действий и меры предосторожности при заболевании
PRESCRIPTION = pd.read_csv(
    PATH + 'my_data.my_symptom_precautions.csv', index_col="Disease"
    )


# df симптомы болезней
SYMPTOMS_OF_DISEASE = pd.read_csv(
    PATH + 'my_data.symptoms_diagnos.csv', index_col='Disease'
    )
